              ...
              <q-tr :props="props">
                ...
                <q-td key="value" :props="props" :class="'text-right'">
                  <q-linear-progress
                    ...
                    class="q-mt-md"
                    size="10px"
                    v-if="props.row.campo == 'Profitability'"
                  >
                    <q-tooltip>
                      ...
                      <p style="font-size: 12px; margin-top: -10px">
                        Won {{ dataInicial["ganadas"] }} out of
                        {{ dataInicial["trades"] }} trades, which represents
                        {{ props.row.value }}
                      </p>
                      <p style="font-size: 12px; margin-top: -10px">
                        Loss {{ dataInicial["perdidas"] }} out of
                        {{ dataInicial["trades"] }} trades, which represents
                        {{ 1 - props.row.value }}
                      </p>
                    </q-tooltip>
          ...
          this.rows3 = [
            {
              campo: "Pips",
              value:
                response.data["pipsGanados"] - response.data["pipsPerdidos"],
            },
            {
              campo: "Best Trade (Pips)",
              value:
                "(" +
                response.data["fechaBestTrade"] +
                ")" +
                " " +
                response.data["bestTradePips"],
            },
            {
              campo: "Worst Trade (Pips)",
              value:
                "(" +
                response.data["worstTradeDate"] +
                ")" +
                " " +
                response.data["worstTradePips"],
            },
          ];
          Loading.hide();
          this.mostrar = true
